...
CREATE TABLE Users (
    ...
    CHECK (cdate >= CURRENT_DATE),
    /* Check if date is valid */
    CHECK (date_part('year', age(dob)) >= 16),
    /* Check if user is 16 y/o and above */
    CHECK (
        /* Check gender */
        gender = 'M'
        OR gender = 'F')
...
CREATE TABLE Passenger (
    uname varchar(15) PRIMARY KEY REFERENCES Users ON DELETE CASCADE,
    mstatus varchar(10) DEFAULT 'MEMBER',
    /* Membership status */
    tpoints integer DEFAULT 0,
    /* Total accumalated reward points (upon joining: all members enjoy 100 points free) */
    cpoints integer DEFAULT 0,
    /* Current points user has after using points for Discount */
    rating numeric(3, 2) DEFAULT 5.00,
    /* Default Passenger rating */
    CHECK (
        /* Ratings: Range of 0 to 5 inclusive */
        rating <= 5
        AND rating >= 0),
    CHECK (tpoints >= 0),
    /* Check if tpoints is valid */
    CHECK (cpoints >= 0),
    /* Check if cpoints is valid */
    CHECK (
        /* Check if mstatus is valid */
        mstatus = 'MEMBER'
        OR mstatus = 'PREMIUM'
        OR mstatus = 'VIP')
);
...
/*---------------------------- 		DRIVER PART 		----------------------------------------*/
CREATE TABLE Car (
    uname varchar(15) REFERENCES Driver ON DELETE CASCADE,
    plate_num integer UNIQUE,
    /* Plate number: Assuming all integers (eg. 1234, 1111, 0000) */
    num_seats integer NOT NULL,
    /* Maximum capacity of the car, excluding driver */
    model varchar(7) NOT NULL,
    edate date NOT NULL,
    /* End date of COE */
    PRIMARY KEY (uname, plate_num),
    CHECK (num_seats > 0),
    /* Check if the mcap is valid */
    CHECK (
        /* Check if the COE expiry: */
        date_part(
            /* Minimum 1 year before expiry to register */
            'day', (edate::timestamp - now()::timestamp)) >= 365)
);
...
CREATE TABLE Ride (
    ...
    plate_num integer,
    pmax integer,
    /* Maximum passengers driver accepts*/
    origin varchar(20),
...
/*---------------------------- 		PASSENGER PART		 ----------------------------------------*/
CREATE TABLE Reward (
    rcode char(7) PRIMARY KEY,
    /* Reward code (Eg. ABC0000) */
    expired boolean DEFAULT FALSE,
    /* States if reward has expired */
    CHECK (
        /* D = Discount, P = Point */
        rcode LIKE 'D%'
        OR rcode LIKE 'R%')
...
CREATE TABLE Obtains (
    ...
    expdate timestamp DEFAULT
    /* Expiry date */
    (CURRENT_TIMESTAMP + interval '3 months'),
    redeemed boolean DEFAULT FALSE,
    /* States if the discount has been redeemed */
    PRIMARY KEY (uname, rcode, expdate),
    CHECK (expdate - interval '3 months' >= CURRENT_TIMESTAMP)
...
CREATE TABLE Transactions (
    ...
    oprice integer,
    /* AMount received by Driver */
    tprice integer,
    /* Total price Passenger paid */
    prating integer,
    /* Rating given to passenger by driver */
    drating integer,
    /* Rating given to driver by passenger */
    ptype varchar(7),
    closed boolean DEFAULT FALSE,
    FOREIGN KEY (puname) REFERENCES Passenger (uname),
    -- FOREIGN KEY (r_issue)
    ...
    CHECK (
        /* Passenger ratings: Range of 0 to 5 inclusive */
        prating <= 5
        AND prating >= 0),
    CHECK (
        /* Driver ratings: Range of 0 to 5 inclusive */
        drating <= 5
        AND drating >= 0)
);
